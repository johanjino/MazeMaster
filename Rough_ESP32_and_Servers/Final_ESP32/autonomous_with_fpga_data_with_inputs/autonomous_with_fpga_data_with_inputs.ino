...
String send_and_receive(int32_t Cam_Data[7], int dir) {
  int32_t Data_To_Send[8] = {0};
  
  for (int i = 0; i<7; i++){
    Data_To_Send[i] = Cam_Data[i];
  }
  Data_To_Send[7] = dir;
...
void loop() {
  ...
  while(Cam_Data[6] == 0){
    ...
    countForward++;
    if (countForward == 3){
      response = (send_and_receive(Cam_Data, 1)).toInt();
  ...
  if(countRight == 5) {
    Serial.println(" TURN ");
    response = (send_and_receive(Cam_Data, 2)).toInt();
    Serial.print(response);
      if (response == 2 ){
        turn_mode = 2;
      }
