...
SmoothStepper::SmoothStepper(int number_of_steps, int motor_pin_1,
                             int motor_pin_2, int R, double F) {
    ...
    //If right motor or not:
    this->R = R;

    //if motor forward or not: F (1) then forward, F (0) then backward:
    this->F = F;
...
/** Makes direction reverse **/
void SmoothStepper::reverseDirection() {
  if(this->F){
    this->F = 0;
  }
  else {
...
void SmoothStepper::MakeForward() {
 this->F = 1.0;
}

void SmoothStepper::MakeBackward() {
 this->F = 0.0;
}
...
void SmoothStepper::stepMotor(int thisStep) {
    if (this->pin_count == 2) {
      if(this->R) { //if right motor wheel
        //F is member var if F is 1 then Forward, F 0 is backward
        if (this->F) {
          switch (thisStep) {
            case 0:  // 01
        ...
        //if left motor
          ...
          if(this->F) {
            switch (thisStep) {
              case 0:  // 01
                  digitalWrite(motor_pin_1, LOW); //HIGH
                  digitalWrite(motor_pin_2, LOW); //LOW
                  break;
              case 1:  // 11
                  digitalWrite(motor_pin_1, HIGH);
                  digitalWrite(motor_pin_2, LOW);
                  break;
              case 2:  // 10
                  digitalWrite(motor_pin_1, HIGH); //LOW
                  digitalWrite(motor_pin_2, HIGH); //HIGH
                  break;
              case 3:  // 00
                  digitalWrite(motor_pin_1, LOW);
                  digitalWrite(motor_pin_2, HIGH);
